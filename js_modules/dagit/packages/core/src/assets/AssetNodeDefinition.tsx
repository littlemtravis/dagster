...
const OpNamesDisplay = (props: {
  ...
  const graphPath = workspacePathFromAddress(repoAddress, `/graphs/${graphName}`);
  const graphURL = `${graphPath}~${opNames.join(',')}`;

  return (
    <Box flex={{gap: 4, alignItems: 'center'}}>
      <Icon name="job" size={16} />
      <Mono>
        <Link to={graphURL}>{graphName}</Link> ({opCount === 1 ? '1 op' : `${opCount} ops`})
      </Mono>
    </Box>
  );
};
